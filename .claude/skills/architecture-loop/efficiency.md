...
- The capture preload renders them: `~/.cache/gent-pass<N>/eff/fetch-capture.ts`, where `<N>` is the latest pass that has one (copy it into this pass's `eff/` directory first). Run `bun --config=apps/tui/bunfig.toml --preload <preload> apps/tui/src/main.tsx -H "<prompt>"` with `CAP_DIR` set to a scratch directory, fake `ANTHROPIC_API_KEY` and `OPENAI_API_KEY`, no `--debug`, and `GENT_DATA_DIR` under the scratch directory (`safety.md` allows this run). The preload saves each provider request body to `CAP_DIR`, answers Anthropic with the scripted SSE steps in `$CAP_DIR/script.json` and OpenAI with a 400, and blocks every other host but the `models.dev` catalog read.
