...
## Final Audit Results (2026-05-11)

8 parallel Explore agents ran the 8 lanes verbatim. Raw lane reports captured in this session's transcript. Triage below separates genuine correctness P0s (must fix before W33 close) from P0-labeled style/hygiene that downgrades to P1.

### Genuine P0s (W33-C13.x sub-commits required)

| #     | Site                                                                                                                                   | Issue                                                                                                                                                                                                                                                                                                                 | Fix shape                                                                                                                                                                                                                |
| ----- | -------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| C13.1 | `packages/core/src/runtime/agent/agent-loop.actor.ts:485-486` + `packages/core/src/runtime/agent/agent-loop.behavior.ts:398,1312-1313` | Plain `let handle: AgentLoopBehavior` and `let started = false` racing under `concurrency: "unbounded"`. The startup semaphore narrows but does not close the torn-read window for `handle` (assigned at 649 outside the semaphore-held region). `started` is non-atomic boolean → double-start of `startTurnWorker`. | Promote both to `Ref` (handle: `Ref<AgentLoopBehavior \| undefined>`; started: `Ref<boolean>`). Read/write inside `startupSemaphore`/`sideMutationSemaphore`. Use `Ref.getAndSet(startedRef, true)` for the start guard. |
| C13.2 | `packages/extensions/src/anthropic/oauth.ts:689`                                                                                       | Module-level `let _env: AnthropicKeychainEnv = {}` mutated by `initAnthropicKeychainEnv`. Hidden global state: last write wins silently across extension instances/hot-reload.                                                                                                                                        | Thread `AnthropicKeychainEnv` into `AnthropicPlatformShape` (it already holds `platform`/`parentEnv`); `buildAnthropicModelDriver` already receives `platform` — add `env` fields there and delete the module-level var. |
| C13.3 | `packages/extensions/src/openai/oauth.ts:1`                                                                                            | `import { Buffer } from "node:buffer"` — banned node import; not in duplication-guard allowlist; no GentPlatform shim.                                                                                                                                                                                                | Replace `Buffer.from(base64, "base64url").toString("utf8")` usage with `new TextDecoder().decode(base64urlDecode(part))` using a pure-JS base64url helper (or surface as `GentPlatform.base64urlDecode`).                |
| C13.4 | `packages/core/src/server/build-fingerprint.ts:40`                                                                                     | `new URL(import.meta.url).pathname` — bare `new URL` equivalent of banned `fileURLToPath`. Slips under guards. File already depends on `GentPlatform`.                                                                                                                                                                | Route through `GentPlatform.fileURLToPath(import.meta.url)` (already added in W33-C10.1).                                                                                                                                |
| C13.5 | `packages/core/src/runtime/file-index/fallback-adapter.ts:46`                                                                          | Module-level mutable `const gitignoreCache = new Map<string, PathMatcher[]>()` — process-global singleton survives across tests/workspaces, defeats Effect scope-based cleanup.                                                                                                                                       | Promote to `Ref.make(new Map())` captured inside the service layer (or `Layer.scoped`-owned `Effect.cached` keyed on cwd).                                                                                               |
| C13.6 | `packages/extensions/src/acp-agents/claude-sdk.ts:129-168`                                                                             | Hand-rolled `Pushable<T>` with `Promise.withResolvers` outside Effect's scheduler — interruption-correctness gap.                                                                                                                                                                                                     | Replace with `Queue.unbounded<T>` (or `TxQueue.unbounded<T>`) + `Stream.fromQueue(...).toAsyncIterable`; keeps the AsyncIterable surface the Claude SDK expects but routes through Effect's scheduler.                   |
| C13.7 | `packages/extensions/src/auto/index.ts:143-186,293`                                                                                    | `journalInterceptorImpl` has a dead `next` indirection — `next` parameter is called once and immediately returned; the runtime already sequences handlers.                                                                                                                                                            | Delete the `next` param; inline `toolResult: (input) => Effect.gen(function* () { ...; return input.result })`.                                                                                                          |

### Lane 7 root-cause (P0-equivalent, fixed as one commit)

| #     | Site                                                                           | Issue                                                                                                                                                                                                                                                                                | Fix shape                                                                                                                                                                                                                                                             |
| ----- | ------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| C13.8 | `packages/core/src/domain/extension-services.ts:169` (`ExtensionFilesService`) | Facet only exposes `listFiles`. Tools that need read/write/exists/stat/resolve `yield* FileSystem.FileSystem` directly, bypassing the facade. 4 P0s in lane 7 (`fs-tools/write.ts`, `fs-tools/edit.ts`, `fs-tools/read-service.ts`, `librarian/repo-explorer.ts`) all converge here. | Extend `ExtensionFilesService` with `read`, `write`, `exists`, `stat`, `resolve`. Migrate the 4 P0 sites to the facet. Re-audit P1 infra sites (skills, journal, vault, oauth) but those layer-construction-time uses are likely the intended pattern (defer to W34). |

### Downgraded to P1 (W34 plan)

- **Queue → TxQueue alignment** (lane 1): `event.ts:367` and `acp-agents/protocol.ts:141` use legacy `Queue.unbounded`. Not a correctness bug — `Queue` and `TxQueue` are both Effect-native; the project simply leans `TxQueue` for new code. Mechanical migration, defer.
- **acp-agents/index.ts dual-path confusion** (lane 4): `buildAcpContributions` returns a full contributions bag but `makeAcpAgentsExtension` re-declares `agents`/`reactions` literally and only delegates `resources`/`externalDrivers`. Dead-codey but not breaking — defer.
- **session-runtime-context.ts hostDeps bag** (lane 8): `ResolveSessionEnvironmentParams` carries `sessionStorage`/`hostDeps` service bag; functions return `Effect<…, never>` yet call `params.sessionStorage.getSession(…)`. Largest residual yield-don't-thread debt. Defer to W34 — fits cleanly with deferred C12 (encore DX) under a single "yield-don't-thread + encore upstream" wave.
- **`Ref<Map>` copy-on-write** (lane 1): `agent-loop.session-governance.ts`, `file-lock.ts`, `session-commands.ts` — 4 sites that could be `TxRef<HashMap>` for STM atomicity. Stylistic; defer.
- **`performance.timeOrigin` in retry.ts:166** (lane 1): use `Clock.currentTimeMillis`. One-line fix, defer.
- **`build-fingerprint.ts:56-57` `Effect.runSync(Effect.cached(...))`** (lane 1): fragile module-top eager eval; move to `Layer.effect` + `Effect.cachedWithTTL`. Defer.
- **File-existence P0s (lane 6)** — `format-schema-error.ts` dead export, `sql-client.ts` orphan from production, `rpcs/runtime.ts` 21-line single-importer. These are file-organization hygiene, not correctness bugs. Defer — bundle with another inline pass in W34.
- **All lane 5 findings** — feed directly into the deferred WAVE-34 encore DX plan.

### Notes

- Lane 5 reports 0 P0s. All upstream library DX debt flows into W34.
- Lane 8 reports 0 P0s — the W33 yield-don't-thread sweep is complete at the "field-level" granularity. Remaining `hostDeps` is the next granularity (P1).
- Total: 8 genuine W33-blocking commits required (C13.1–C13.8). After C13 lands clean, re-run audit lanes 1-3 spot-check to confirm fixes; then close Wave 33.

## Commit 13: close audit P0s
