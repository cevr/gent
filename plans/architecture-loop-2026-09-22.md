...
| Cell namespace                | `2f8880e2` on `cell-namespace`; review found five P2s (probe keys, `null` input, `describe` shadowing, optional marks, prompt size); fix in progress with the stale source-run worker (`packages/core/dist/gent-cell`)                                                                                                                                                                                                                       |
| Background bash as a wake job | owner decision: adds a persisted `WakeEntry` tag and abandons `background_bash_jobs`                                                                                                                                                                                                                                                                                                                                                         |
| `Files`/`Process` facets      | next, after cell namespace: extensions yield `FileSystem`, `Path`, `ChildProcessSpawner`; five shipped extensions already do                                                                                                                                                                                                                                                                                                                 |
