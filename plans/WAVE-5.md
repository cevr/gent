...
## Final Verification Receipt (C14)

Wave 5 implementation completed through commit `c56a1192` (C13 fixup).
Final recursive verification ran the same eight audit lanes at HEAD `c56a1192`:

- `bun run gate` green at `c56a1192` (typecheck + lint + build + 18 tests pass)
- `bun run test:e2e` green at `c56a1192` (37 pass / 0 fail / 105 expect calls)
- Eight independent audit lanes all completed

**Status**: NOT clean. Recursive audit surfaced 9 P1 findings and 25+ P2 findings
across runtime ownership, AI/provider boundaries, domain modeling, storage,
extension API, SDK/TUI, suppression discipline, and test taxonomy. Wave 5
text is preserved below for traceability. Wave 6 plan is in `PLAN.md`.

### Wave 5 commit log (chronological, oldest first)

- `0bde6052` docs(plan): add recursive hardening wave 5
- `c9c57b7a` C1: fix(server): serialize descendant delete against child create
- `0664c09c` C2: fix(capabilities): give transport-public actions wide context
- `d52de3ce` C3: fix(runtime): reject deleted sessions at public read boundaries
- `7d5abfc7` C3 fixup: close C3 check-then-use hole; preserve typed session-not-found
- `6acc5157` C4: fix(extensions): scope-resolve machine actors; reject raw-loaded malformed contributions
- `83781622` C5: fix(providers): fail closed when no credential source is available
- `8185115b` C5 fixup: propagate ProviderAuthError to RPC seam; close google/mistral holes
- `d644892e` C6: fix(runtime): persist external-driver tool calls/results into transcript
- `9ef1c888` C6 fixup: normalize external tool-failed payload; honor driverKind on stream failure
- `d6aaf088` C7: fix(storage): composite FK for task branch + classify keychain errors
- `e34b2b07` C7 fixup: harden task-tools migration against Opus review
- `33334366` C8: fix(domain): tighten tagged-enum/protocol guards; dedup ExtensionActorStatusInfo
- `5107bb11` C9: refactor(extensions): migrate RunSpec callers to makeRunSpec
- `0c74dce8` C10: fix(tui): derive agent model/cost from transport snapshot
- `5918c67e` C10 fixup: freeze cost on StreamEnded at emit time
- `7ee94259` chore(gate): auto-fmt instead of fmt:check
- `e4328dad` C11: fix(boundaries): decode JSON at persistence/OAuth/ACP boundaries
- `09a72564` C12: fix(sdk,tui): identity-probe server stop + idempotent session create/send
- `dcedca96` C12 fixup: concurrent-safe dedup + /new side-effect parity
- `9be99006` C13: test: per-cwd auth/event routing + ACP codemode coverage
- `c56a1192` C13 fixup: tighten two-cwd absence check + fix Omit type
