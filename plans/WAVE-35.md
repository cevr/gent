...
Steer/Queue invariants verified: `agent-loop.state.ts:194-220`
(steering drains before followUp) + `agent-loop.actor.ts:861-925` (only
`applySteer` calls `interruptActiveStream`; `enqueueMessage` only appends to
`queue.followUp`).
