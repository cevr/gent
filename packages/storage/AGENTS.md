...
- **No auto-migrations** - `CREATE TABLE IF NOT EXISTS` doesn't add new columns. Delete old DB or add `ALTER TABLE` for schema changes.
...
messages(id, session_id, branch_id, role, parts, created_at, turn_duration_ms)  -- parts is JSON, turn_duration_ms on user messages
