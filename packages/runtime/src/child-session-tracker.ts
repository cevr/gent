 ...
 * Bootstraps from storage for completed children (history replay).
 */
...
export interface ChildSessionTrackerService {
  /** Start tracking children for a parent session/branch. Bootstraps from storage + subscribes to live events. */
...
export const make: Effect.Effect<
  ChildSessionTrackerService,
  never,
  EventStore | Storage | Scope.Scope
> = Effect.gen(function* () {
  ...
  const service: ChildSessionTrackerService = {
    track: ({ sessionId, branchId }) =>
      Effect.gen(function* () {
        // Bootstrap from storage — seed completed children.
        // Event history to know toolCallId comes from the subscription replay below.
        yield* storage
          .getChildSessions(sessionId)
          .pipe(Effect.catchEager((e) => Effect.logWarning("child session bootstrap failed", e)))

        // Subscribe to parent event stream — replays all past events too
