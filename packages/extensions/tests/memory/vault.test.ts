...
const fallbackIsoDate = "2026-01-01T00:00:00Z"
describe("MemoryVault", () => {
  ...
  vaultTest("write creates parent directories", () =>
    Effect.gen(function* () {
      ...
      expect(yield* fs.exists(path.join(tmpDir, "project/my-proj-abc123/deep-topic.md"))).toBe(true)
    }),
  )
  ...
  vaultTest("rebuildIndex creates scope and root indexes", () =>
    Effect.gen(function* () {
      ...
      const fs = yield* FileSystem.FileSystem
      const path = yield* Path.Path
      const rootIndex = yield* fs.readFileString(path.join(tmpDir, "index.md"))
      const scopeIndex = yield* fs.readFileString(path.join(tmpDir, "global/index.md"))
