...
export const runCommand = (cmd: ReadonlyArray<string>) => {
  const [first, ...rest] = cmd
  const head = Option.fromNullishOr(first)
  if (Option.isNone(head)) return Effect.succeed("")
  return Effect.gen(function* () {
    const ctx = yield* ExtensionContext
    return yield* ctx.Process.run(head.value, rest, {
      cwd: ctx.cwd,
      stdout: "pipe",
      stderr: "pipe",
    })
