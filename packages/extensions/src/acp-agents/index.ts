...
const codemodeInstructions = (toolList: string): string =>
  [
    ...
    "JavaScript that runs in the session's persistent cell. Host tools are",
    "`await tools.call('<name>', args)`; the last expression is the result.",
    "",
    "```",
    'await tools.call(\'grep\', { pattern: "TODO", path: "src/" })',
    "```",
    "",
    "Compose multiple calls in one `execute` invocation. Top-level variables",
    "survive between invocations:",
    "",
    "```",
    "const files = await tools.call('glob', { pattern: \"**/*.ts\" })",
    "const matches = []",
    "for (const f of files) {",
    "  matches.push(await tools.call('grep', { pattern: \"FIXME\", path: f }))",
    "}",
    "matches",
    "```",
    "",
