...
): Option.Option<PromptSection> => {
  const lines: string[] = []

  if (Option.isSome(projectKey)) {
    const projectEntries = entries.filter((e) => e.path.startsWith(`project/${projectKey.value}/`))
    if (projectEntries.length > 0) {
      lines.push(`### Project: ${projectDisplayName(projectKey.value)}`)
...
interface VaultProjectionValue {
  ...
  readonly projectKey: Option.Option<string>
}
...
> = Effect.gen(function* () {
  ...
  const key = Option.fromNullishOr(yield* projectKeyOf(ctx.cwd))
  const failed = (e: Error): ProjectionError =>
    new ProjectionError({
  ...
  const projectEntries = yield* vault
    .list("project", Option.getOrUndefined(key))
    .pipe(Effect.catchEager((e) => Effect.fail(failed(e))))
