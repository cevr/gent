...
describe("Task Storage", () => {
  it.live("createTask + getTask roundtrip", () =>
    Effect.gen(function* () {
  ...
  it.live("getTask returns undefined for missing", () =>
    Effect.gen(function* () {
      const { storage } = yield* setup
  ...
  it.live("listTasks by session + branch", () =>
    Effect.gen(function* () {
      const { storage } = yield* setup
      yield* storage.createTask(makeTask("t1"))
  ...
  it.live("updateTask changes status", () =>
    Effect.gen(function* () {
      const { storage } = yield* setup
      yield* storage.createTask(makeTask("t1"))
  ...
  it.live("updateTask returns undefined for missing", () =>
    Effect.gen(function* () {
      const { storage } = yield* setup
  ...
  it.live("deleteTask removes task", () =>
    Effect.gen(function* () {
      const { storage } = yield* setup
      yield* storage.createTask(makeTask("t1"))
  ...
  it.live("task with metadata roundtrips", () =>
    Effect.gen(function* () {
      const { storage } = yield* setup
...
describe("Task Dependencies", () => {
  it.live("addTaskDep + getTaskDeps", () =>
    Effect.gen(function* () {
      const { storage } = yield* setup
      yield* storage.createTask(makeTask("t1"))
      yield* storage.createTask(makeTask("t2"))
  ...
  it.live("deleting task removes its own deps", () =>
    Effect.gen(function* () {
      const { storage } = yield* setup
      yield* storage.createTask(makeTask("t1"))
      yield* storage.createTask(makeTask("t2"))
  ...
  it.live("duplicate dep is idempotent", () =>
    Effect.gen(function* () {
      const { storage } = yield* setup
      yield* storage.createTask(makeTask("t1"))
      yield* storage.createTask(makeTask("t2"))
