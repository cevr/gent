...
describe("MemoryVault", () => {
  ...
  test("write creates parent directories", async () => {
    const vault = makeMemoryVault(tmpDir)
    await Effect.runPromise(
      vault.write("project/my-proj-abc123/deep-topic.md", makeFm("project"), "# Deep\n\nNested."),
    )
    expect(Fs.existsSync(Path.join(tmpDir, "project/my-proj-abc123/deep-topic.md"))).toBe(true)
  })

  test("list returns entries with parsed frontmatter", async () => {
    const vault = makeMemoryVault(tmpDir)
    await Effect.runPromise(
      Effect.gen(function* () {
        yield* vault.ensureDirs()
  ...
  test("remove deletes file and rebuilds index", async () => {
    const vault = makeMemoryVault(tmpDir)
    await Effect.runPromise(
      Effect.gen(function* () {
        yield* vault.ensureDirs()
  ...
  test("rebuildIndex creates scope and root indexes", async () => {
    const vault = makeMemoryVault(tmpDir)
    await Effect.runPromise(
      Effect.gen(function* () {
        yield* vault.ensureDirs()
