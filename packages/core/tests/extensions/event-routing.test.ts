...
describe("EventPublisher — event routing", () => {
  ...
  it.live("ExtensionUiSnapshot does not recurse — even with an actor that reacts to it", () => {
    ...
    return Effect.gen(function* () {
      ...
      // The queued publisher should:
      // 1. append TurnCompleted → reduce fires → state may change → snapshot published
      // 2. append the snapshot directly to BaseEventStore, without re-entering reduce
