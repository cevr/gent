...
describe("ResourceManager", () => {
  test("read needs for the same tag can overlap", async () => {
    const maxRunning = await runWithResourceManager(
      Effect.gen(function* () {
  ...
  test("write needs exclude reads for the same tag", async () => {
    const writeStartedWhileReadHeld = await runWithResourceManager(
      Effect.gen(function* () {
        const manager = yield* ResourceManager
  ...
  test("write need wins when a tool declares both read and write for one tag", async () => {
    const maxRunning = await runWithResourceManager(
      Effect.gen(function* () {
        const manager = yield* ResourceManager
        const running = yield* Ref.make(0)
  ...
  test("write need wins when declared before a duplicate read need", async () => {
    const maxRunning = await runWithResourceManager(
      Effect.gen(function* () {
        const manager = yield* ResourceManager
        const running = yield* Ref.make(0)
