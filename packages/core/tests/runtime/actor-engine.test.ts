...
describe("ActorEngine — pure reducer", () => {
  it.live("Behavior.receive folds Inc into +1", () =>
    Effect.gen(function* () {
      const ctx = {
      ...
      const next = yield* counterBehavior.receive(
        CounterMsg.Inc.make({}),
        { count: 5 },
        ctx,
      ) as Effect.Effect<CounterState, unknown, never>
      expect(next.count).toBe(6)
    }),
  )
})
describe("ActorEngine — runtime", () => {
  it.live("spawn + tell evolves state, observable via ask", () =>
    Effect.gen(function* () {
      yield* Effect.scoped(
        Effect.gen(function* () {
  ...
  it.live("ask returns ActorAskTimeout when actor is unknown", () =>
    Effect.gen(function* () {
  ...
  it.live("ask times out when no reply arrives", () =>
    Effect.gen(function* () {
  ...
  it.live("tell to an unknown ref is a no-op (does not hang or fail)", () =>
    Effect.gen(function* () {
      yield* Effect.scoped(
        Effect.gen(function* () {
          const engine = yield* ActorEngine
  ...
  it.live("messages are delivered in submission order to a single actor", () =>
    Effect.gen(function* () {
...
describe("ActorEngine — subscribeState", () => {
  it.live("emits initial state then post-receive state on every change", () =>
    Effect.gen(function* () {
  ...
  it.live("dedupes consecutive equal states (filter-changed semantics)", () =>
    Effect.gen(function* () {
  ...
  it.live("subscribeState on unknown ref is empty (no hang, no fail)", () =>
    Effect.gen(function* () {
      yield* Effect.scoped(
        Effect.gen(function* () {
          const engine = yield* ActorEngine
...
describe("ActorEngine — peekView", () => {
  it.live("samples behavior.view(state) at the post-receive state", () =>
    Effect.gen(function* () {
      interface ViewState {
        readonly count: number
      }
  ...
  it.live("returns undefined when behavior declares no view", () =>
    Effect.gen(function* () {
      yield* Effect.scoped(
        Effect.gen(function* () {
          const engine = yield* ActorEngine
  ...
  it.live("returns undefined for unknown ref (matches tell no-op semantics)", () =>
    Effect.gen(function* () {
      yield* Effect.scoped(
        Effect.gen(function* () {
          const engine = yield* ActorEngine
