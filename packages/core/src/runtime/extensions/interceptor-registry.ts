 ...
 *
 * Owns the contribution-side interceptor composition algorithm: scope-ordered
 * (builtin → user → project, then id-stable), left-fold over each key's chain
 * with defect isolation around every interceptor body.
 *
 * @module
 */
...
/**
 * Compile interceptor contributions from loaded extensions into a chain map.
 *
 * Scope ordering: builtin → user → project (inner to outer via left fold).
 * Interceptor defects fall through to the next handler with a warning log.
 */
