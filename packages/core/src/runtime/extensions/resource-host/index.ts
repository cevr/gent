...
import type { ResourceSubscription } from "../../../domain/resource.js"
...
/**
 * Collect every subscription declared by every Resource. Used by the
 * server composition root to pre-populate the {@link SubscriptionEngine}
 * Layer at process startup.
 */
export const collectSubscriptions = (
  extensions: ReadonlyArray<LoadedExtension>,
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
): ReadonlyArray<ResourceSubscription<any>> =>
  extensions.flatMap((ext) =>
    extractResources(ext.contributions).flatMap((r) => r.subscriptions ?? []),
  )
