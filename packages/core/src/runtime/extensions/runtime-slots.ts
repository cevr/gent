...
import type { CompiledPipelines } from "./pipeline-host.js"
import type { CompiledSubscriptions } from "./subscription-host.js"
...
export const compileRuntimeSlots = (
  ...
  return {
    normalizeMessageInput: (input, ctx) =>
      pipelines.runPipeline("message.input", input, (state) => Effect.succeed(state.content), ctx),

    checkPermission: (input, base, ctx) =>
      pipelines.runPipeline("permission.check", input, base, ctx),

    resolveContextMessages: (input, ctx) =>
      Effect.gen(function* () {
        const legacy = yield* pipelines.runPipeline(
          "context.messages",
          input,
