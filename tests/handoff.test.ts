...
describe("HandoffHandler", () => {
  describe("Test layer", () => {
    test("returns sequential decisions", async () => {
    ...
    test("defaults to confirm", async () => {
    ...
    test("respond returns undefined (no-op in test)", async () => {
      const layer = HandoffHandler.Test()
  ...
  describe("Live layer", () => {
    ...
    test("present/respond confirm flow returns entry with summary", async () => {
      await Effect.runPromise(
        Effect.gen(function* () {
          const handler = yield* HandoffHandler
    ...
    test("present/respond reject flow returns entry", async () => {
      await Effect.runPromise(
        Effect.gen(function* () {
          const handler = yield* HandoffHandler
          const eventStore = yield* EventStore
    ...
    test("respond to unknown requestId returns undefined", async () => {
      await Effect.runPromise(
        Effect.gen(function* () {
          const handler = yield* HandoffHandler
