...
describe("executeShell", () => {
  ...
  test("executes simple command", async () => {
    const result = await executeShell("echo hello", testDir)
  ...
  test("captures stderr", async () => {
    const result = await executeShell("echo error >&2", testDir)
    expect(result.output).toContain("error")
    expect(result.truncated).toBe(false)
  })

  test("respects cwd", async () => {
    const result = await executeShell("pwd", testDir)
  ...
  test("handles multi-line output", async () => {
    const result = await executeShell("echo -e 'line1\\nline2\\nline3'", testDir)
  ...
  test("handles empty output", async () => {
    const result = await executeShell("true", testDir)
    expect(result.output).toBe("")
    expect(result.truncated).toBe(false)
  })

  test("handles command with arguments", async () => {
    const result = await executeShell("echo -n test", testDir)
    expect(result.output).toBe("test")
  })

  test("handles pipes", async () => {
    const result = await executeShell("echo hello | tr 'h' 'H'", testDir)
    expect(result.output).toBe("Hello")
  })
  ...
  test("handles file operations", async () => {
    ...
    const result = await executeShell(`cat ${testFile}`, testDir)
    expect(result.output).toBe("file content")
  })

  test("truncates output over line limit", async () => {
    // Generate output with more than 2000 lines
    const result = await executeShell("seq 1 2500", testDir)
