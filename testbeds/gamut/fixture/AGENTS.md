...
- If the `delegate` tool is available, you are the orchestrator. You plan,
  delegate, verify, and report. You do not edit source files yourself except to
  resolve a merge between children's results.
- If the `delegate` tool is absent, you are a worker. Do the one task in your
  prompt, nothing else, and return. Never run `bun gamut.ts`.
...
3. Delegate all independent tasks concurrently from one cell with `Promise.all`.
4. After the children return, run `bun test` and `bun run typecheck` yourself.
5. Report: one line per task with pass/fail, and the final test count.

## Child agents

Every `delegate` call must set `overrides` from the roster below. Do not invent
other models.

<!-- roster -->
- Worker (fix or feature): `overrides.modelId` = `openai/gpt-5.6-luna`, `overrides.reasoningEffort` = `max`
