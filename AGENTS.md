...
- **Ephemeral runtime service capture** - `buildEphemeralLayer` in `agent-runner.ts` filters parent services via `Context.omit(...)` before forwarding (so locally-owned services like `Storage`, `EventStore`, `EventPublisher`, `WorkflowRuntime` are not pre-captured from the parent), then `Layer.fresh()` wraps the composed layer at `Effect.provide` to defeat the parent's `MemoMap`. The extension substrate (`buildExtensionLayers`) is shared with server / per-cwd composition roots; only the per-run local services (in-memory storage, auto-resolve approval, fresh AgentLoop) are ephemeral-specific. Getting the omit set wrong, or skipping `Layer.fresh`, causes child events to silently leak into the parent's reduction.
