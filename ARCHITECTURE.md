...
The skills listing goes into every request, so its size is a per-step input cost: 59 skills are about 5.4k tokens, about 45% of the static prefix. `GENT_SKILLS_LISTING=compact` names each skills directory once and gives each skill its name and lead sentence (about 110 characters); the file is `<directory>/<name>/SKILL.md` unless the line names another file. The model reads the full text through the same read path. On the owner's home this cuts the listing from 21.5k to 6.7k characters. The default stays `full`. The measurement that decides the default: the SKILL.md read rate per session (19 of 600 sessions read one before the flag), and the gamut tasks that should trigger a skill, run with the flag off and on. If the compact listing keeps both at the full listing's level, `compact` becomes the default.
