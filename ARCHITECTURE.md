...
- Alarms (`@gent/wake`) are branch-scoped and in-memory: the `wake` tool captures the session facade of its call, a timer forked into the branch resource scope queues a user-role `wake` message with `wake: true` at the due time, and the loop answers it as a new turn. Closing the branch or restarting the server drops pending alarms; that is the known gap, since branch resources start without an `ExtensionContext`.
