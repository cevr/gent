...
- Alarms (`@gent/wake`) live in `~/.gent/wakes/<branchId>.json`; timers are branch-scoped. The `wake` tool writes the entry, captures the session facade of its call, and forks a timer into the branch resource scope that queues a user-role `wake` message (`details.note`) with `wake: true` at the due time; firing removes the entry. Branch resources start without an `ExtensionContext`, so after a branch close or a server restart the stored alarms get their timers back on the branch's next turn (the `turnProjection` hook re-arms them; past-due ones fire at once). The TUI collapses a `wake` row to `⏰ alarm fired · <note>`.
