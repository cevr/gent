...
- A row that is not a message comes from `noticeRowContribution`: the extension derives it per branch from `transport.onSessionEvent`, and the session view merges it into the transcript by time. Nothing stores it and the model never reads it. The session feed opens only after every client extension has loaded, so each one sees the branch's replay from the first event; a reconnect or a remount repeats envelope ids, which the subscriber skips.
- `@gent/cache` (`cache.client.tsx`) folds a branch's stream, tool, interaction, message and compaction events into prompt-cache misses above a 1,024-token noise floor. Each miss gets a cause: a model switch, a changed prefix inside the 5-minute TTL (the regression alarm for a moved cache marker), or an expiry during a tool call, an approval wait, a paused turn, before a child completion, before a wake, or after idle time. The cost is the step's paid input or write rate less the catalog's cache-read rate (`transport.models()`). A row shows a miss of 20k tokens or $0.10; the status row shows the branch's `cache waste $X`. The row is telemetry for the reader, not context for the model.
